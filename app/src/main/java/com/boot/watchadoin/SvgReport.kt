...
class SvgReport(val timelines: List<Timeline>, htmlEmbed: Boolean = false) {
  ...
  init {
    ...
    totalDurationMs =
        timelines
            .map { it.duration.toLongMilliseconds() + it.relativeStart.toLongMilliseconds() }
            .maxBy { it }
    ...
    timelines.forEachIndexed { index, timeline ->
      ...
      val rowTimelines = rects[heightIndex] ?: ArrayList<Rect>().apply { rects[heightIndex] = this }
      rowTimelines += rect
    ...
    svgHeight =
        ((rowCount + 1) * padding + rowCount * timelineHeight + xAxisHeight + padding).toLong()

    var output =
        """<svg id="stopwatch" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 $svgWidthNormalized $svgHeight" width="$svgWidthNormalized" height="$svgHeight">"""
...
@ExperimentalTime
fun Stopwatch.asSvgReport(timeaxisPlaceholder: Boolean = false): SvgReport =
    SvgReport(timelines(includeParent = true), timeaxisPlaceholder)
...
private infix fun Timeline.collidesWith(other: Timeline): Boolean {
  ...
  return start.between(otherStart, otherEnd) ||
      end.between(otherStart, otherEnd) ||
      otherStart.between(start, end) ||
...
internal val xmlEscapeMap =
    mapOf('"' to "&quot;", '\'' to "&apos;", '<' to "&lt;", '>' to "&gt;", '&' to "&amp;")

internal fun String.escapeXml(): String {
  val sb = StringBuilder()

  forEach { char -> sb.append(xmlEscapeMap[char] ?: char) }
...
<script type="text/javascript">
    ...
    report.timelines.mapIndexed { index, timeline ->
      """d("${timeline.name.escapeXml()}", ${timeline.tid}, ${timeline.duration}, ${
        report.relations[index].joinToString(
          separator = ",",
          prefix = "[",
          postfix = "]"
        )
      }, ${report.ancestors[index].joinToString(separator = ",", prefix = "[", postfix = "]")})"""
