...
class CharMatcher(private val word: String) {
	...
	private val balloonIndex get() = runningIndex % word.length
	fun matchNext(character: Char): Boolean {
		return (character == word[balloonIndex]).also { isMatch ->
			println("runningIndex % word.length = balloonIndex -> $runningIndex % ${word.length} = $balloonIndex")
