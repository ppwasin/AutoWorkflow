...
inline val DefaultMeasurePolicy:
	MeasureScope.(
...
fun Modifier.animateMovement(
	...
	var targetOffsetAnimation:
		Animatable<IntOffset, AnimationVector2D>? by remember {
		mutableStateOf(null)
	}
		...
		this@composed
			...
			.intermediateLayout { measurable, constraints, _ ->
				...
				layout(placeable.width, placeable.height) {
					val (x, y) =
						(targetOffsetAnimation?.value ?: targetOffset!!) - placementOffset
					placeable.place(x, y)
				}
			}
	}
...
// for test
fun Modifier.transformation(lookaheadScope: LookaheadLayoutScope) =
	with(lookaheadScope) {
