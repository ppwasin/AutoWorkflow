...
fun count(vararg nums: Int): Int {
    return if(nums.isEmpty()) 0
    else 1 + count(*nums.drop(1).toIntArray())
}

fun sum(nums: Array<Int>, size: Int): Int {
    return if(size <= 0) 0
    else nums[size-1] + sum(nums, size-1)
}

fun count(nums: Array<Int>, size: Int): Int {
    return if(size <= 0) 0
    else 1 + count(nums, size - 1)
}
...
fun max(nums: Array<Int>, size: Int): Int {
    ...
    return if(nums[size-1] > maxValue) nums[size-1]
    else maxValue
}
...
fun binaryRecursion(
    ...
    return when{
        value == target -> value
        target < value -> binaryRecursion(sortedValues, max = guessMiddle, min = min, target)
        target > value -> binaryRecursion(sortedValues, max = max, min = guessMiddle, target)
        else -> null
    }
